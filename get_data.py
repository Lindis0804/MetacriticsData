...
def get_data(i, serial):
    ...
    href = list(map(lambda a: a.get_attribute("href"), browser.find_elements(By.CSS_SELECTOR, 'a[class="title"]')))
    for index in range(serial, len(href)):
        ...
        try:
            ...
            sleep(5)
            # try:
            image = browser.find_element(By.CSS_SELECTOR, image_css).get_attribute("src")
            # except:
            #     image = ""
            try:
                rating_list = browser.find_elements(By.CSS_SELECTOR, rating_list_css)
            except selenium.common.exceptions.NoSuchElementException:
                rating_list = [-1, -1, -1, -1, -1, -1]
            try:
                year = int(convert(browser.find_element(By.CSS_SELECTOR, year_css).text))
            except:
                year = -1
            try:
                title = browser.find_element(By.CSS_SELECTOR, title_css).find_element(By.TAG_NAME, 'h1').text
            except selenium.common.exceptions.NoSuchElementException:
                title = ""
            try:
                num_of_critic = int(
            ...
            for x in critic_score_css:
                ...
                if (critic_scores != []):
                    ...
                    break
            try:
                num_of_rating = int(convert(rating_list[1].text.replace('based on ', '').replace(' Ratings', '')))
            except:
                num_of_rating = -1
            rating_score = -1.0
            ...
            try:
                ...
                facts = []
            try:
                summary = browser.find_element(By.CSS_SELECTOR, summary_css).text.replace('Summary: ', '')
            except selenium.common.exceptions.NoSuchElementException:
                summary = ""
            try:
            ...
            try:
                ...
                genres = []
            try:
                rating = browser.find_element(By.CSS_SELECTOR, rating_css).text.replace("Rating:", '')
            except selenium.common.exceptions.NoSuchElementException:
                rating = ""
            try:
                runtime = int(convert(
